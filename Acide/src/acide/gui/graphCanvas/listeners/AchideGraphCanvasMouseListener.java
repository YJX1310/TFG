                ...
                for (Node n : nodes) {
                    if (ev.getX() >= n.getX()
                        ...
                        if(n.getLabel().contains("/")) {
                            String view = n.getLabel().split("/")[0];

                            LinkedList<String> info = AcideDatabaseManager.getInstance().getSelectAll(AcideDatabaseManager.getInstance().currentDB(), view);

                            if(!info.isEmpty() && !info.get(0).equals("$error")) {
                                AcideDatabaseDataView panelDv = AcideMainWindow.getInstance()
                                        .getDataBasePanel().getDataView(AcideDatabaseManager.getInstance().currentDB(), view);
                                panelDv.build(info);
                                panelDv.setAlwaysOnTop(true);
                                //panelDv.setAlwaysOnTop(false);
                                panelDv.setVisible(true);
                            }else{
                               info = AcideDatabaseManager.getInstance().getAllTuples(n.getLabel());

                                if (!info.isEmpty() && !info.get(0).equals("$error")) {
                                    AcideDatabaseDataView panelDv  = AcideMainWindow.getInstance()
                                            .getDataBasePanel().getDataView(AcideDatabaseManager.getInstance().currentDB(), n.getLabel());
                                    panelDv.build(info);
                                    panelDv.setAlwaysOnTop(true);
                                    //panelDv.setAlwaysOnTop(false);
                                    panelDv.setVisible(true);
