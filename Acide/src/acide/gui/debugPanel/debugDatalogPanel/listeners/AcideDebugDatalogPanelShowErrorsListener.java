...
/**
 * ACIDE - A Configurable ID debug datalog panel show errors listener.
 *
 * @see ActionListener
 * @version 0.19
 *
 */
public class AcideDebugDatalogPanelShowErrorsListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(AcideDebugDatalogDebugWindow.getInstance().getErrors().size()>0){
            info += "<br>" + AcideLanguageManager.getInstance().getLabels()
                    .getString("s2365");
            for(String line: AcideDebugDatalogDebugWindow.getInstance().getErrors().values()){
                info += "<br>" + " " + " " + " - " + line;
            }
        }
        info += "</html>";
